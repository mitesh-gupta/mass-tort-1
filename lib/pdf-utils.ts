...
export async function createMergedPDF(
  ...
  try {
    ...
    const photoIdType = photoIdFile.type || "image/jpeg"; // Default to jpeg if type is missing
    ...
    if (photoIdType === "application/pdf") {
      ...
      try {
        if (photoIdType.includes("png")) {
          ...
          // Default to JPEG for any other image type (including camera photos)
          image = await pdfDoc.embedJpg(photoIdBytes);
        }
      } catch (error) {
        console.error("Error embedding image:", error);
        throw new Error(
          "Failed to process photo. Please try taking another photo or selecting a different file."
        );
      }
    ...
    console.error("Error in createMergedPDF:", error);
